...
// A new order alone does NOT reduce this counter. A scanned item on a ramp changes to staged and then disappears from this counter.
(() => {
  ...
  function copy() {
    const l = langCode();
    if (l === "uk") return { title: "НА СКЛАДІ · БЕЗ РАМПИ", loading: "Завантаження…" };
    if (l === "pl") return { title: "NA MAGAZYNIE · POZA RAMPĄ", loading: "Ładowanie…" };
    return { title: "PÅ LAGER · UTEN RAMPE", loading: "Laster…" };
  }
  ...
  function ensureStyle() {
    ...
    style.textContent = `
      #bamaCompactStockCounter{
        width:100%;max-width:100%;margin:12px 0 16px;padding:14px 16px 16px;
        border:2px solid #303b59;border-radius:24px;background:#0d1426;color:#f5f7ff;
        box-shadow:0 10px 24px rgba(0,0,0,.14);font-family:Arial,Helvetica,sans-serif;
      }
      ...
      @media(max-width:420px){
        #bamaCompactStockCounter{padding:12px 12px 14px;border-radius:20px}
        #bamaCompactStockCounter .bcsc-grid{gap:8px}
        #bamaCompactStockCounter .bcsc-cell{min-height:102px;border-radius:17px;padding:11px 5px}
      }
    `;
  ...
  function preferredAnchor() {
    ...
    const h1 = document.querySelector("main.app h1, main h1, .app h1, h1");
    return h1 || null;
  }
  ...
  function ensureCard() {
    ...
    card.innerHTML = `
      <div class="bcsc-head">
        <div class="bcsc-title" id="bcscTitle"></div>
        <div class="bcsc-time" id="bcscTime"></div>
      </div>
      <div class="bcsc-grid">
  ...
  function paintLabels() {
    ...
    card.querySelectorAll("[data-bcsc-product]").forEach(cell => {
      cell.classList.toggle("active", cell.dataset.bcscProduct === active);
    });
  }
  ...
  async function fetchRows() {
    try {
      if (typeof request === "function") {
        return await request("mottak_scans?select=product,status,stock_status&limit=10000") || [];
      }
    } catch (error) {
      throw error;
    }
  ...
  function setTime(value) {
    const node = document.getElementById("bcscTime");
    if (node) node.textContent = value;
  }

  function nowText() {
    try {
      return new Intl.DateTimeFormat("nb-NO", { hour: "2-digit", minute: "2-digit", second: "2-digit" }).format(new Date());
    } catch {
  ...
  async function refresh() {
    ...
    try {
      ...
      const inStock = rows.filter(row => row.status === "verified" && (row.stock_status || "in_stock") === "in_stock");
      const counts = {
        bunner: inStock.filter(row => row.product === "bunner").length,
        hyller30: inStock.filter(row => row.product === "hyller30").length,
        hyller60: inStock.filter(row => row.product === "hyller60").length
      };
      document.getElementById("bcscB").textContent = String(counts.bunner);
      document.getElementById("bcscH30").textContent = String(counts.hyller30);
      document.getElementById("bcscH60").textContent = String(counts.hyller60);
      setTime(nowText());
  ...
  document.addEventListener("click", event => {
    if (event.target.closest?.("#products,[data-language],#noBtn,#uaBtn,#utLanguageSwitch")) {
      setTimeout(() => { paintLabels(); refresh(); }, 80);
    }
  }, true);
