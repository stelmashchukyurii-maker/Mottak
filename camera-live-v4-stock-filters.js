...
(() => {
  ...
  style.textContent = `
    ...
    @media(max-width:760px){
      .camera-stock-filters{grid-template-columns:repeat(5,minmax(145px,1fr))}
  ...
  function rowsForStatus(filter) {
    if (filter === "physical") return rows.filter(row => stockStatus(row) !== "dispatched");
  ...
  renderEditRow = function renderEditStockRow(row) {
    return `<tr class="editing" data-edit-row="${esc(row.id)}">
      ...
      <td class="edit-cell">
        <input type="hidden" data-edit-field="upper_number" value="${SYSTEM_UPPER}">
  ...
  loadTable(false);
  console.info("Camera Cloud v4.9 stock and product filters are active; fixed upper number is hidden.");
})();
