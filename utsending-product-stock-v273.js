...
(() => {
  ...
  style.textContent = `
    ...
    #utProductStockV273 .ups-grid{
      display:grid;
      grid-template-columns:repeat(3,minmax(0,1fr));
      gap:7px;
    }
    #utProductStockV273 .ups-cell{
      min-width:0;
      padding:9px 6px;
      border:1px solid #303b59;
      border-radius:11px;
    ...
    #utProductStockV273 .ups-cell span{
      display:block;
      color:#aab4ce;
      font-size:10px;
      font-weight:950;
    ...
    #utProductStockV273 .ups-cell strong{
      display:block;
      margin-top:5px;
      color:#f5f7ff;
      font-size:24px;
  ...
  function ensureCard() {
    ...
    card.innerHTML = `
      <div class="ups-head">
        <span>PÅ LAGER · UTEN RAMPE</span>
        <span id="upsTime">Laster…</span>
      </div>
      <div class="ups-grid">
  ...
  async function load() {
    ...
    try {
      ...
      const b = rows.filter(row => row.product === "bunner").length;
      const h30 = rows.filter(row => row.product === "hyller30").length;
      const h60 = rows.filter(row => row.product === "hyller60").length;

      document.getElementById("upsB").textContent = String(b);
      document.getElementById("upsH30").textContent = String(h30);
      document.getElementById("upsH60").textContent = String(h60);
      ...
      if (error) {
        error.textContent = "Kunne ikke oppdatere lagerantall.";
