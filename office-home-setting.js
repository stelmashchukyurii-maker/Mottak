...
(() => {
  if (window.__BAMA_WORKING_FRESH_ALL__) return;
  window.__BAMA_WORKING_FRESH_ALL__ = true;
  ...
  function freshHref(href, currentToken = token()) {
    try {
      ...
      u.searchParams.set("v", currentToken);
      u.searchParams.set("fresh", currentToken);
  ...
  function ensureButton() {
    if (!document.getElementById("bamaWorkingFreshStyle")) {
      const style = document.createElement("style");
      style.id = "bamaWorkingFreshStyle";
      style.textContent = `
        #bamaWorkingFreshAll{position:fixed;right:max(12px,env(safe-area-inset-right));bottom:max(14px,calc(env(safe-area-inset-bottom) + 8px));z-index:99999;min-height:56px;padding:10px 16px;border:2px solid #75b7ff;border-radius:999px;background:#10223b;color:#eef7ff;font:1000 13px/1.15 Arial,sans-serif;box-shadow:0 14px 34px rgba(0,0,0,.48);touch-action:manipulation}
        #bamaWorkingFreshAll:active{transform:translateY(1px)}
        #bamaWorkingFreshAll[disabled]{opacity:.65}
      `;
      document.head.appendChild(style);
    }
    if (document.getElementById("bamaWorkingFreshAll")) return;
    const button = document.createElement("button");
    button.id = "bamaWorkingFreshAll";
    button.type = "button";
    button.textContent = "↻ ОНОВИТИ ВСЕ";
    ...
    button.addEventListener("click", async () => {
      ...
      try { sessionStorage.setItem(TOKEN_KEY, nextToken); } catch {}
      try {
        if ("caches" in window) {
          const keys = await caches.keys();
          await Promise.all(keys.map(key => caches.delete(key)));
        }
      } catch (error) { console.warn("[WORKING FRESH] Cache Storage", error); }
      const u = new URL(location.href);
      u.search = "";
      u.searchParams.set("v", nextToken);
      u.searchParams.set("fresh", nextToken);
      location.replace(u.toString());
    });
  }

  function applyFreshAll() {
    const changed = applyTokenToPages();
    if (changed && typeof render === "function") render();
    document.querySelectorAll('a[href^="index.html"]').forEach(link => { link.href = freshHref("index.html"); });
    ensureButton();
  }

  document.addEventListener("bama:office-switch-rendered", () => queueMicrotask(applyFreshAll));
  if (document.readyState === "loading") document.addEventListener("DOMContentLoaded", applyFreshAll, { once: true });
  else applyFreshAll();
})();
