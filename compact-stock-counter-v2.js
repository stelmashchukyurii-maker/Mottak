...
// Safe compact warehouse counter shared by WORKING and GREEN.
// Business rule 09.08.2026:
// counter = current in_stock MINUS the still-unscanned quantity of all active ramp orders.
// This avoids double subtraction: once an item is scanned to a ramp it becomes staged,
// leaves in_stock, and also counts as fulfilled for that ramp order.
...
(() => {
  ...
  function langCode(){
    ...
    try{
      const u=localStorage.getItem("mottak_ut_language");
      if(["nb","pl","uk"].includes(u))return u;
      const c=localStorage.getItem("mottak_cloud_v4_language");
      if(["nb","pl","uk"].includes(c))return c;
    }catch{}
    return "nb";
  }
  ...
  function titleText(){
    const l=langCode();
    return l==="uk"?"НА СКЛАДІ · МІНУС РАМПИ":l==="pl"?"MAGAZYN · MINUS RAMPY":"PÅ LAGER · MINUS RAMPER";
  }

  function ensureStyle(){
    if(document.getElementById("bamaCompactStockStyleV2"))return;
    const s=document.createElement("style");
    s.id="bamaCompactStockStyleV2";
    s.textContent=`
...
`;
  ...
  function paint(){
    const c=ensureCard();
    if(!c)return;
    const t=document.getElementById("bcscTitle"),want=titleText();
    if(t&&t.textContent!==want)t.textContent=want;
    const ap=activeProduct();
    c.querySelectorAll("[data-bcsc-product]").forEach(x=>x.classList.toggle("active",x.dataset.bcscProduct===ap));
  }

  async function requestJson(path){
    if(typeof request==="function")return await request(path)||[];
    if(typeof client!=="undefined"&&client?.from){
      if(path.startsWith("mottak_scans?")){
        const r=await client.from("mottak_scans").select("product,status,stock_status,ut_order_id").limit(10000);
        if(r.error)throw r.error;
        return r.data||[];
      }
      if(path.startsWith("ut_orders?")){
        const r=await client.from("ut_orders").select("id,status,bunner_stacks,hyller30_sets,hyller60_sets").limit(1000);
        if(r.error)throw r.error;
        return r.data||[];
      }
    }
  ...
  function calculate(scans,orders){
    ...
    verified.forEach(r=>{
      if(!r.ut_order_id||r.stock_status!=="staged")return;
      const id=String(r.ut_order_id);
      if(!stagedByOrder.has(id))stagedByOrder.set(id,{bunner:0,hyller30:0,hyller60:0});
      const bucket=stagedByOrder.get(id);
      if(Object.prototype.hasOwnProperty.call(bucket,r.product))bucket[r.product]+=1;
    });

    const stillForRamps={bunner:0,hyller30:0,hyller60:0};
    active.forEach(o=>{
      const used=stagedByOrder.get(String(o.id))||{bunner:0,hyller30:0,hyller60:0};
      stillForRamps.bunner+=Math.max(0,qty(o,"bunner")-used.bunner);
      stillForRamps.hyller30+=Math.max(0,qty(o,"hyller30")-used.hyller30);
      stillForRamps.hyller60+=Math.max(0,qty(o,"hyller60")-used.hyller60);
    });

    return {
      bunner:Math.max(0,inStock.bunner-stillForRamps.bunner),
      hyller30:Math.max(0,inStock.hyller30-stillForRamps.hyller30),
      hyller60:Math.max(0,inStock.hyller60-stillForRamps.hyller60)
    };
  }

  function set(id,v){const e=document.getElementById(id);if(e&&e.textContent!==String(v))e.textContent=String(v)}

  function patchOfficeAvailable(v){
    const box=document.querySelector(".office-stock-row.available-row .office-stock-products");
    if(!box)return;
    const html=`<span><b>Bunner:</b> ${v.bunner} ${v.bunner===1?"stabel":"stabler"}</span><span><b>Hyller x30:</b> ${v.hyller30} sett</span><span><b>Hyller x60:</b> ${v.hyller60} sett</span>`;
    if(box.innerHTML!==html)box.innerHTML=html;
  }

  async function refresh(){
    paint();
    if(busy||!ensureCard())return;
    busy=true;
    try{
      const [scans,orders]=await Promise.all([
        requestJson("mottak_scans?select=product,status,stock_status,ut_order_id&limit=10000"),
        requestJson("ut_orders?select=id,status,bunner_stacks,hyller30_sets,hyller60_sets&limit=1000")
      ]);
      const v=calculate(scans,orders);
      set("bcscB",v.bunner);set("bcscH30",v.hyller30);set("bcscH60",v.hyller60);
      set("bcscTime",new Date().toLocaleTimeString("nb-NO",{hour:"2-digit",minute:"2-digit",second:"2-digit"}));
      patchOfficeAvailable(v);
    }catch(e){
      set("bcscTime","—");
      console.warn("Compact stock v2 refresh failed",e);
    }finally{busy=false}
  }

  function loop(){
    clearTimeout(timer);
    timer=setTimeout(async()=>{await refresh();loop()},REFRESH_MS);
  }
